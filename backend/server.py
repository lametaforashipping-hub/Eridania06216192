...
@api_router.get("/admin/high-risk-tickets")
...
@api_router.put("/lotteries/{lottery_id}/prize-tiers")
...
async def create_ticket(ticket: TicketCreate, current_user: dict = Depends(get_current_user)):
    ...
    # === HIGH RISK ALERT NOTIFICATION (Simple Tickets) ===
...
@api_router.get("/monitoring/live-tickets")
async def get_live_tickets(
    ...
    """Get the most recent tickets in real-time for Super Admin"""
    query = {}
    ...
    # If since_id is provided, get tickets created after that one
    if since_id:
...
@api_router.get("/admin/seller-profile/{seller_id}")
...
@api_router.post("/admin/act-as-seller")
