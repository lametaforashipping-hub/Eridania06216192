...
async def get_receipt_image(ticket_number: str):
    """Generate a complete receipt image"""
    ...
    lottery_id_to_name = {}
    for lot in await db.lotteries.find({}, {"id": 1, "name": 1}).to_list(100):
        lottery_id_to_name[lot.get("id", "")] = lot.get("name", "LOTERÍA")
    
    png_bytes = generate_receipt_image(ticket, company, lottery_id_to_name)
