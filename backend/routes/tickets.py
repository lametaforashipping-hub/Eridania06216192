...
async def get_receipt_image(ticket_number: str):
    ...
    try:
        ...
        font_slogan = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-Italic.ttf", 18)
        font_ticket_num = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf", 22)
        font_date = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf", 16)
        font_lottery_name = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf", 18)
        font_play = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf", 16)
        font_total = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf", 26)
        font_footer = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf", 14)
        font_phone = ImageFont.truetype("/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf", 14)
    except Exception:
    ...
    # === SLOGAN (italic, purple color like reference image) ===
    ...
    if created:
        ...
        if created:
            ...
            if hour > 12:
                ...
                hour = 12
            date_str = f"{created_dr.day:02d}/{created_dr.month:02d}/{created_dr.year}, {hour}:{created_dr.minute:02d} {am_pm}"
    ...
    for lottery_name, lottery_plays in plays_by_lottery.items():
        ...
        draw.text((margin, y), lottery_header, fill='#1a1a2e', font=font_lottery_name)
        y += 28
        
        # Each play under this lottery: format "P 04-20 = RD$20"
        for play in lottery_plays:
            ...
            # Format exactly like reference: "  P 04-20 = RD$20"
            play_line = f"  {abbr} {numbers_str} = {currency_display}{int(amount)}"
            draw.text((margin, y), play_line, fill='#1a1a2e', font=font_play)
            y += 26
    ...
    # === TOTAL (large, bold, centered) ===
    total = ticket.get("total_amount", 0)
    total_text = f"TOTAL: {currency_display} {total:.2f}"
    bbox = draw.textbbox((0, 0), total_text, font=font_total)
    text_w = bbox[2] - bbox[0]
    draw.text(((width - text_w) // 2, y), total_text, fill='#1a1a2e', font=font_total)
    y += 50
    ...
    text_w = bbox[2] - bbox[0]
    draw.text(((width - text_w) // 2, y), footer_text, fill='#1a1a2e', font=font_footer)
    y += 24
