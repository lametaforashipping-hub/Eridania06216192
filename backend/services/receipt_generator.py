"""Receipt image generator service using Pillow"""
import io
import base64
from datetime import datetime, timedelta, timezone
from collections import defaultdict
from PIL import Image, ImageDraw, ImageFont
import qrcode


BOLD_FONT = "/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf"

PLAY_TYPE_ABBR = {
    "quiniela": "Q", "pale": "P", "tripleta": "T",
    "super_pale": "SP", "first": "1ra", "second": "2da", "third": "3ra"
}

LOGO_BASE64 = ""


def _load_fonts():
    """Load all bold fonts for receipt"""
    try:
        return {
            "company": ImageFont.truetype(BOLD_FONT, 26),
            "address": ImageFont.truetype(BOLD_FONT, 15),
            "label": ImageFont.truetype(BOLD_FONT, 15),
            "ticket_num": ImageFont.truetype(BOLD_FONT, 22),
            "date": ImageFont.truetype(BOLD_FONT, 15),
            "play_row": ImageFont.truetype(BOLD_FONT, 14),
            "total_label": ImageFont.truetype(BOLD_FONT, 17),
            "total_amount": ImageFont.truetype(BOLD_FONT, 24),
            "footer": ImageFont.truetype(BOLD_FONT, 15),
        }
    except Exception:
        default = ImageFont.load_default()
        return {k: default for k in ["company", "address", "label", "ticket_num", "date", "play_row", "total_label", "total_amount", "footer"]}


def _draw_centered(draw, y, text, font, fill, width):
    """Draw centered text and return new y"""
    bbox = draw.textbbox((0, 0), text, font=font)
    text_w = bbox[2] - bbox[0]
    draw.text(((width - text_w) // 2, y), text, fill=fill, font=font)


def _draw_right_aligned(draw, y, text, font, fill, width, margin):
    """Draw right-aligned text"""
    bbox = draw.textbbox((0, 0), text, font=font)
    text_w = bbox[2] - bbox[0]
    draw.text((width - margin - text_w, y), text, fill=fill, font=font)


def _paste_logo(img, draw, y, width):
    """Paste the golden ball logo at the top, returns new y"""
    try:
        logo_data = base64.b64decode(LOGO_BASE64)
        logo_img = Image.open(io.BytesIO(logo_data))

        if logo_img.mode == 'RGBA':
            bg = Image.new('RGB', logo_img.size, 'white')
            bg.paste(logo_img, mask=logo_img.split()[3])
            logo_img = bg
        elif logo_img.mode != 'RGB':
            logo_img = logo_img.convert('RGB')

        logo_img = logo_img.resize((80, 80), Image.Resampling.LANCZOS)
        img.paste(logo_img, ((width - 80) // 2, y))
        return y + 90
    except Exception as e:
        print(f"[RECEIPT] Logo load error: {e}")
        return y + 20


def _format_date(created):
    """Format ticket creation date in DR timezone 12h format"""
    if not created:
        return None
    if isinstance(created, str):
        try:
            created = datetime.fromisoformat(created.replace('Z', '+00:00'))
        except Exception:
            return None
    dr_tz = timezone(timedelta(hours=-4))
    created_dr = created.astimezone(dr_tz) if created.tzinfo else created
    hour = created_dr.hour
    am_pm = "a. m." if hour < 12 else "p. m."
    hour_12 = hour if hour <= 12 else hour - 12
    if hour_12 == 0:
        hour_12 = 12
    return f"{created_dr.day:02d}/{created_dr.month:02d}/{created_dr.year}, {hour_12}:{created_dr.minute:02d} {am_pm}"


def generate_receipt_image(ticket: dict, company: dict, lottery_id_to_name: dict) -> bytes:
    """
    Generate a PNG receipt image for a ticket.
    Returns raw PNG bytes.
    """
    fonts = _load_fonts()
    width = 420
    margin = 25
    row_height = 24

    # Company info
    company_name = (company.get("company_name") or "LOTERIA MAGICA") if company else "LOTERIA MAGICA"
    company_address = (company.get("address") or "SANTO DOMINGO") if company else "SANTO DOMINGO"
    company_city = (company.get("city") or "DISTRITO NACIONAL") if company else "DISTRITO NACIONAL"
    company_rnc = (company.get("rnc") or "123-456-789") if company else "123-456-789"

    # Group plays by lottery name
    plays = ticket.get("plays", [])
    plays_by_lottery = defaultdict(list)
    for play in plays:
        lottery_name = play.get("lottery_name")
        if not lottery_name or lottery_name == "LOTERÍA":
            lottery_id = play.get("lottery_id", "")
            lottery_name = lottery_id_to_name.get(lottery_id, "LOTERÍA")
        plays_by_lottery[lottery_name].append(play)

    ticket_currency = ticket.get("currency", "RD$")
    currency_display = "US$" if ticket_currency in ["USD", "US$", "US"] else "RD$"
    ticket_number = ticket.get("ticket_number", "")

    # Estimate height
    total_plays = len(plays)
    estimated_height = 120 + 120 + 100 + (total_plays * row_height) + 40 + 80 + 180 + 100

    img = Image.new('RGB', (width, estimated_height), 'white')
    draw = ImageDraw.Draw(img)
    y = 20

    # === LOGO ===
    y = _paste_logo(img, draw, y, width)

    # === COMPANY NAME ===
    _draw_centered(draw, y, company_name.upper(), fonts["company"], '#1a237e', width)
    y += 32

    # === ADDRESS ===
    for line in [company_address.upper(), company_city.upper(), f"RNC: {company_rnc}"]:
        _draw_centered(draw, y, line, fonts["address"], '#666666', width)
        y += 20
    y += 15

    # === SEPARATOR ===
    draw.line([(margin, y), (width - margin, y)], fill='#cccccc', width=1)
    y += 25

    # === TICKET NUMBER ===
    _draw_centered(draw, y, "NO. BOLETO", fonts["label"], '#3949ab', width)
    y += 22
    _draw_centered(draw, y, ticket_number, fonts["ticket_num"], '#1a1a2e', width)
    y += 30

    # === DATE ===
    date_str = _format_date(ticket.get("created_at"))
    if date_str:
        _draw_centered(draw, y, date_str, fonts["date"], '#666666', width)
        y += 25
    y += 10

    # === SEPARATOR ===
    draw.line([(margin, y), (width - margin, y)], fill='#cccccc', width=1)
    y += 20

    # === PLAYS TABLE HEADER ===
    draw.text((margin, y), "LOTERÍA", fill='#3949ab', font=fonts["play_row"])
    draw.text((margin + 190, y), "NÚM.", fill='#3949ab', font=fonts["play_row"])
    _draw_right_aligned(draw, y, "MONTO", fonts["play_row"], '#3949ab', width, margin)
    y += 20
    draw.line([(margin, y), (width - margin, y)], fill='#cccccc', width=1)
    y += 8

    # === PLAYS ROWS ===
    all_plays = []
    for lottery_name, lottery_plays in plays_by_lottery.items():
        for play in lottery_plays:
            all_plays.append((lottery_name, play))

    for i, (lottery_name, play) in enumerate(all_plays):
        if i % 2 == 0:
            draw.rectangle([(margin - 5, y - 2), (width - margin + 5, y + row_height - 4)], fill='#f5f5f5')

        play_type = play.get("lottery_type", "quiniela")
        abbr = PLAY_TYPE_ABBR.get(play_type, play_type[:1].upper())
        lottery_short = (lottery_name or "Lotería")[:20]
        draw.text((margin, y), f"[{abbr}] {lottery_short}", fill='#1a1a2e', font=fonts["play_row"])

        numbers = play.get("numbers", [])
        numbers_str = "-".join(str(n).zfill(2) for n in numbers) if isinstance(numbers, list) else str(numbers)
        draw.text((margin + 190, y), numbers_str, fill='#1a1a2e', font=fonts["play_row"])

        amount_text = f"{currency_display} {int(play.get('amount', 0))}"
        _draw_right_aligned(draw, y, amount_text, fonts["play_row"], '#1a1a2e', width, margin)
        y += row_height

    y += 15 if all_plays else 10

    # === TOTAL ===
    total = ticket.get("total_amount", 0)
    draw.text((margin, y), f"TOTAL ({len(all_plays)} jugadas)", fill='#1a1a2e', font=fonts["total_label"])
    _draw_right_aligned(draw, y - 3, f"{currency_display} {total:.2f}", fonts["total_amount"], '#3949ab', width, margin)
    y += 50

    # === QR CODE ===
    qr = qrcode.QRCode(version=1, box_size=5, border=2)
    qr.add_data(ticket_number)
    qr.make(fit=True)
    qr_img = qr.make_image(fill_color="black", back_color="white").convert('RGB')
    qr_w, qr_h = qr_img.size
    img.paste(qr_img, ((width - qr_w) // 2, y))
    y += qr_h + 30

    # === FOOTER ===
    _draw_centered(draw, y, "CONSERVE ESTE BOLETO", fonts["footer"], '#3949ab', width)
    y += 22
    _draw_centered(draw, y, "¡BUENA SUERTE!", fonts["footer"], '#3949ab', width)
    y += 25

    # Crop and export
    img = img.crop((0, 0, width, y + 10))
    buf = io.BytesIO()
    img.save(buf, format='PNG', optimize=True)
    buf.seek(0)
    return buf.getvalue()
